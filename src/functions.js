...
function parseGit(logArray) {
    ...
    {
      data = logArray[i].split(" ");

	//sets the data to the GitLog
		var stfu = new GitLog();	
		
		stfu.hash = String(data[0]);
		
		var screwIt = new Date((String(data[1]) + " " + String(data[2]) + " " + String(data[3])+ " " + String(data[4])+ " " + String(data[5])+ " " + String(data[6])));		
		stfu.date = new Date(screwIt);		
		
		stfu.message = "";		
		for (var j = 7; j < data.length; j++) {              
          stfu.message += (data[j]) + " ";}
		stfu.message = stfu.message.substr(1);
		stfu.message = stfu.message.slice(0,-2);
	//sets the GitLog in a bin			
		bin.push(stfu);}
